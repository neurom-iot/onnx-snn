...
MODEL_SAVE_FOLDER_PATH = '../../model/dnn/lenet-5/'
...
model.add(Conv2D(input_shape = (input_shape[0], input_shape[1], 1), filters = 6, kernel_size = (5, 5), strides=(1, 1), padding = 'same', name = 'conv1'))
model.add(Activation('tanh'))
model.add(AveragePooling2D(pool_size = (2, 2), strides = (2, 2), name = 'avgpool1'))

model.add(Conv2D(filters = 16, kernel_size = (5, 5), strides=(1, 1), padding = 'valid', name = 'conv2'))
model.add(Activation('tanh'))
model.add(AveragePooling2D(pool_size = (2, 2), strides = (2, 2), name = 'avgpool2'))

model.add(Conv2D(filters = 120, kernel_size = (5, 5), strides=(1, 1), padding = 'valid', name = 'conv3'))
model.add(Activation('tanh'))

model.add(Flatten(name = 'flatten'))

model.add(Dense(84, name = 'fullconnected1'))
model.add(Activation('tanh'))
...
model.save(MODEL_SAVE_FOLDER_PATH + "lenet-5_tanh.h5")